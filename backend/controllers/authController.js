...
// ── Local disk storage for user profile images ───────────────────
...
exports.sendRegistrationOTP = async (req, res) => {
  try {
    ...
    let user = existing;
    if (!user) {
      user = new User({ firstName: firstName.trim(), lastName: lastName.trim(), email: email.toLowerCase().trim(), phone: phone?.trim() || '', role: 'user', status: 'active', isActive: true });
    } else {
...
exports.verifyRegistrationOTP = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'No registration found for this email.', error: 'USER_NOT_FOUND' });
    if (user.isEmailVerified) return res.status(400).json({ message: 'Account already verified. Please sign in.' });
    if (!user.emailVerificationOTP) return res.status(400).json({ message: 'No OTP requested. Please send OTP first.', error: 'NO_OTP_REQUESTED' });
    if (user.isOTPBlocked()) {
      const mins = Math.ceil((user.otpBlockedUntil - new Date()) / 60000);
      return res.status(429).json({ message: `Too many attempts. Try again in ${mins} minutes.` });
    }
...
exports.sendAdminRegistrationOTP = async (req, res) => {
  try {
    ...
    let admin = existing;
    if (!admin) {
      admin = new User({ firstName: firstName.trim(), lastName: lastName.trim(), email: email.toLowerCase().trim(), role: 'admin', status: 'active', isActive: true });
    } else {
      admin.firstName = firstName.trim(); admin.lastName = lastName.trim(); admin.role = 'admin';
    }
...
exports.verifyAdminRegistrationOTP = async (req, res) => {
  try {
    ...
    if (!admin) return res.status(404).json({ message: 'Admin registration not found.' });
    if (admin.isOTPBlocked()) return res.status(429).json({ message: 'Account temporarily blocked.' });
    const valid = admin.verifyEmailOTP(otp);
    if (!valid) { await admin.save(); return res.status(400).json({ message: 'Invalid or expired OTP.' }); }
...
exports.sendGuideRegistrationOTP = async (req, res) => {
  try {
    ...
    let user = existing;
    if (!user) {
      user = new User({ firstName: firstName.trim(), lastName: lastName.trim(), email: email.toLowerCase().trim(), phone: phone.trim(), role: 'guide', status: 'pending', isActive: false });
    } else {
      user.firstName = firstName.trim(); user.lastName = lastName.trim();
      user.phone = phone.trim(); user.role = 'guide'; user.status = 'pending';
    }
    if (user.isOTPBlocked()) {
      const mins = Math.ceil((user.otpBlockedUntil - new Date()) / 60000);
      return res.status(429).json({ message: `Too many attempts. Try again in ${mins} minutes.` });
...
exports.verifyGuideRegistrationOTP = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'No guide registration found.', error: 'USER_NOT_FOUND' });
    if (!user.emailVerificationOTP) return res.status(400).json({ message: 'No OTP requested.', error: 'NO_OTP_REQUESTED' });
    if (user.isOTPBlocked()) {
      const mins = Math.ceil((user.otpBlockedUntil - new Date()) / 60000);
      return res.status(429).json({ message: `Too many attempts. Try again in ${mins} minutes.` });
    }
    ...
    await Guide.findOneAndUpdate(
      { user: user._id },
      { user: user._id, yearsExperience: yearsExperience || 0, specializations: specializations || [], languages: languages || [], bio: bio || '', licenseNumber: guideLicense?.trim() || '', applicationStatus: 'pending' },
      { upsert: true, new: true }
    );
    return res.status(201).json({ success: true, message: 'Guide application submitted! You will hear back within 2-3 business days.', user: { id: user._id, email: user.email, role: user.role, status: user.status } });
  } catch (err) {
...
exports.login = async (req, res) => {
  try {
    ...
    if (status === 'pending')   return res.status(403).json({ success: false, message: 'Your account is pending approval.' });
    if (status === 'suspended') return res.status(403).json({ success: false, message: 'Your account has been suspended.' });
    const token = signToken(user);
...
// ── Update profile: name, phone, profileImage (file or URL) ──────
