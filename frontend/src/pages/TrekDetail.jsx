...
export default function TrekDetail() {
  ...
  return (
    <><style>{S}</style>
        ...
        <div className="td-body">
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                {allImgs.slice(0, 6).map((img, i) => (
                  <div key={i} onClick={() => setActiveImg(i)}
                    style={{ width: 72, height: 56, borderRadius: 8, overflow: 'hidden', cursor: 'pointer', border: `2px solid ${activeImg === i ? '#16a34a' : 'transparent'}`, flexShrink: 0 }}>
          ...
          <div className="td-sidebar">
            <div className="td-price-card">
              ...
              {/* ✅ Opens BookingModal with eSewa instead of linking to browse-guides */}
              <button
            ...
            {trek.region && (
              <Link to={`/destinations/${trek.region.slug}`}
                style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #e5f0e8', borderRadius: 12, padding: '14px 16px', textDecoration: 'none', color: '#0f172a', fontSize: 13, fontWeight: 600, transition: 'all 0.2s' }}>
                <span>🗺</span>
                <div>
      ...
      {/* ✅ BookingModal — handles trek booking + eSewa redirect */}
      {showBookingModal && (
        <BookingModal
          type="trek"
          item={trek}
