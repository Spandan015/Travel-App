...
function AltitudeChart({ itinerary }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="td-alt-svg" preserveAspectRatio="none">
      ...
      {points.map((p, i) => (
        <circle key={i} cx={xScale(p.day)} cy={yScale(p.el)} r="4" fill="#16a34a" stroke="#fff" strokeWidth="1.5"/>
      ))}
      {points.map((p, i) => (
        <text key={i} x={xScale(p.day)} y={H - 4} textAnchor="middle" fontSize="9" fill="#94a3b8">Day {p.day}</text>
      ))}
    </svg>
  );
}
  ...
  return (
    ...
    <div className="td-root">
      ...
      <section className="td-hero">
        <div className="td-hero-bg" style={{ backgroundImage: heroImg ? `url(${getImg(heroImg)})` : 'none', background: heroImg ? undefined : 'linear-gradient(135deg,#0a2818,#1a4a2a)' }} />
      ...
      <div className="td-body">
              ...
              <div className="td-inc-grid">
                {trek.priceIncludes?.filter(Boolean).length > 0 && (
                  <div>
                    ...
                    <ul className="td-inc-list">
                      {trek.priceIncludes.filter(Boolean).map((item,i) => (
                        <li key={i} className="td-inc-item"><span className="td-inc-dot">✓</span>{item}</li>
                      ))}
                    </ul>
                  </div>
                )}
                {trek.priceExcludes?.filter(Boolean).length > 0 && (
                  <div>
                    ...
                    <ul className="td-inc-list">
                      {trek.priceExcludes.filter(Boolean).map((item,i) => (
                        <li key={i} className="td-inc-item"><span className="td-inc-dot" style={{color:'#dc2626'}}>✗</span>{item}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        <div className="td-sidebar">
          <div className="td-price-card">
            ...
            <div className="td-price-val">NPR {(trek.price || 0).toLocaleString()}</div>
            <div className="td-price-per">per person</div>
            {trek.priceUSD && <div className="td-price-usd">≈ USD {trek.priceUSD.toLocaleString()}</div>}
            <Link to="/browse-guides" className="td-book-btn">Book This Trek</Link>
          ...
          {trek.region && (
            <Link to={`/destinations/${trek.region.slug}`} style={{display:'flex',alignItems:'center',gap:8,background:'#fff',border:'1px solid #e5f0e8',borderRadius:12,padding:'14px 16px',textDecoration:'none',color:'#0f172a',fontSize:13,fontWeight:600,transition:'all 0.2s'}}>
              <span>🏔</span>
              <div>
