...
const STYLES = `
  ...
  .pd-day-line { width:2px; background:var(--border); margin:4px 0; flex-shrink:0; align-self:stretch; margin-left:19px; }
  ...
  .pd-counter-wrap { border:2px solid var(--border); border-radius:10px; padding:12px 14px; margin-bottom:14px; }
  .pd-counter-name { font-size:13px; font-weight:600; color:var(--text); }
  .pd-counter-btns { display:flex; align-items:center; gap:12px; }
  .pd-counter-btn { width:28px; height:28px; border-radius:50%; border:1.5px solid var(--border); background:#fff; font-size:16px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-weight:600; color:var(--text); transition:all 0.15s; line-height:1; }
  .pd-counter-btn:hover:not(:disabled) { border-color:var(--blue); color:var(--blue); }
  .pd-counter-btn:disabled { opacity:0.3; cursor:not-allowed; }
  .pd-counter-val { font-size:14px; font-weight:700; color:var(--text); min-width:20px; text-align:center; }
  .pd-breakdown { background:var(--bg); border-radius:10px; padding:14px; margin-bottom:14px; }
...
export default function PackageDetails() {
  ...
  const calcEndDate = () => {
    if (!startDate || !pkg?.duration) return '';
    const d = new Date(startDate);
    d.setDate(d.getDate() + Number(pkg.duration));
  ...
  const allImages = pkg ? [pkg.mainImage, ...(pkg.images||[])].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i) : [];
      ...
      <div className="pd-root">
        ...
        <div className="pd-header-wrap">
          ...
          <div className="pd-meta-row">
            {pkg.difficulty && (
              <span className="pd-diff-badge" style={{background:DIFF_BG[pkg.difficulty]||'#F1F5F9',color:DIFF_COLOR[pkg.difficulty]||'#374151'}}>
                {pkg.difficulty}
              </span>
            )}
                  ...
                  pkg.itinerary.map((day, i) => (
                    <div key={i} className="pd-day">
                      <div style={{display:'flex',flexDirection:'column',alignItems:'center',flexShrink:0}}>
                        ...
                        {i < pkg.itinerary.length-1 && <div className="pd-day-line" style={{flex:1,minHeight:20}} />}
                      </div>
              ...
              <>
                <div className="pd-section">
                  ...
                  <div className="pd-includes-grid">
                    ...
                    {pkg.includes?.activities?.map(a => (
                      <div key={a} className="pd-include-item">✅ {a}</div>
                    ))}
                  </div>
                    ...
                    {(() => {
                      const mapMarkers = pkg.destinations
                        ...
                        .map((d,i)=>({
                          ...
                          title:d.name||'Destination', description:d.location||'', primary:i===0,
                        }));
            ...
            <div className="pd-book-card">
              {success ? (
                <div className="pd-success">
                  ...
                  <p>Your booking for <strong>{name}</strong> has been submitted successfully.</p>
                  <Link to="/dashboard" className="pd-success-btn">View My Bookings</Link>
                </div>
              ) : (
                <form onSubmit={handleBooking}>
                  ...
                  <div className="pd-book-note-top">📅 {pkg.duration} days · {pkg.maxGroupSize ? `Max ${pkg.maxGroupSize} people` : 'Flexible group'}</div>
                  ...
                  {startDate && pkg.duration && (
                    <div style={{fontSize:12,color:'var(--muted)',marginTop:-8,marginBottom:12}}>
                      Ends: {new Date(calcEndDate()).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}
                    </div>
                  )}
                  ...
                  <div className="pd-counter-wrap">
                    <div className="pd-counter-row">
                      <div className="pd-counter-name">Travelers</div>
                      <div className="pd-counter-btns">
                        <button type="button" className="pd-counter-btn" disabled={guests<=1} onClick={()=>setGuests(v=>v-1)}>−</button>
                        <span className="pd-counter-val">{guests}</span>
                        <button type="button" className="pd-counter-btn" disabled={guests>=(pkg.maxGroupSize||15)} onClick={()=>setGuests(v=>v+1)}>+</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pd-breakdown">
                    ...
                    <div className="pd-pb-total">
                      <span>Total</span>
                      <span>NPR {totalPrice.toLocaleString()}</span>
                    </div>
                  </div>

                  <button type="submit" className="pd-reserve-btn" disabled={bookingLoading||!startDate}>
                    {bookingLoading ? '⏳ Booking…' : `Book Now · NPR ${totalPrice.toLocaleString()}`}
                  </button>
                  <p className="pd-book-foot">📅 {pkg.duration} day journey · Free cancellation</p>
