...
const STYLES = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  .bg-root{font-family:'Roboto',sans-serif;background:#f8faf8;min-height:100vh;padding-top:68px;}
  .bg-hero{background:linear-gradient(135deg,#0a2818 0%,#0d3320 40%,#1a4a2a 70%,#0a1a10 100%);padding:72px 24px 0;text-align:center;position:relative;overflow:hidden;}
  .bg-hero::before{content:'';position:absolute;inset:0;background:url('https://images.unsplash.com/photo-1554274311-5efce2b2ea3c?w=1400&q=60') center/cover;opacity:0.08;}
  .bg-hero-mountains{position:absolute;bottom:0;left:0;right:0;height:40%;clip-path:polygon(0% 100%,8% 65%,15% 72%,22% 50%,30% 65%,38% 35%,45% 52%,52% 22%,60% 48%,67% 38%,74% 58%,82% 28%,90% 50%,100% 38%,100% 100%);background:rgba(255,255,255,0.03);}
  .bg-hero-content{position:relative;z-index:2;max-width:680px;margin:0 auto;padding-bottom:48px;}
  .bg-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,0.1);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.15);border-radius:100px;padding:6px 16px;font-size:12px;font-weight:500;color:rgba(255,255,255,0.85);letter-spacing:0.05em;text-transform:uppercase;margin-bottom:20px;}
  .bg-badge span{width:6px;height:6px;border-radius:50%;background:#4ade80;display:inline-block;}
  .bg-hero h1{font-size:clamp(2.2rem,5vw,3.8rem);font-weight:700;color:#fff;margin:0 0 16px;line-height:1.1;letter-spacing:-0.02em;}
  ...
  .bg-stats{display:flex;justify-content:center;gap:48px;padding:20px 24px;background:rgba(255,255,255,0.06);border-top:1px solid rgba(255,255,255,0.08);position:relative;z-index:2;}
  .bg-stat{text-align:center;color:white;}
  .bg-stat-num{font-size:1.5rem;font-weight:700;display:block;}
  ...
  .bg-sidebar{background:white;border-radius:16px;border:1px solid #e5f0e8;padding:24px;height:fit-content;position:sticky;top:88px;box-shadow:0 2px 12px rgba(22,163,74,0.06);}
  .bg-sidebar-title{font-size:1rem;font-weight:700;color:#0a2818;margin:0 0 20px;}
  ...
  .bg-filter-input{width:100%;padding:9px 12px;border:1.5px solid #d1fae5;border-radius:9px;font-size:0.875rem;font-family:'Roboto',sans-serif;color:#111827;outline:none;transition:border 0.15s;}
  .bg-filter-input:focus{border-color:#16a34a;}
  .bg-filter-select{width:100%;padding:9px 12px;border:1.5px solid #d1fae5;border-radius:9px;font-size:0.875rem;font-family:'Roboto',sans-serif;color:#111827;outline:none;background:white;cursor:pointer;}
  .bg-reset{width:100%;padding:9px;border:1.5px solid #d1fae5;border-radius:9px;background:transparent;color:#6b7280;font-size:0.83rem;font-weight:500;cursor:pointer;font-family:'Roboto',sans-serif;margin-top:4px;transition:all 0.15s;}
  ...
  .bg-results-count{font-size:1.1rem;font-weight:700;color:#0a2818;}
  .bg-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:22px;}
  .bg-card{background:white;border-radius:18px;border:1px solid #e5f0e8;overflow:hidden;transition:all 0.3s;box-shadow:0 2px 8px rgba(22,163,74,0.05);}
  .bg-card:hover{transform:translateY(-5px);box-shadow:0 16px 48px rgba(22,163,74,0.14);}
  .bg-card-head{background:linear-gradient(135deg,#0a2818,#1a4a2a);padding:24px;display:flex;align-items:center;gap:16px;}
  .bg-avatar{width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#16a34a,#4ade80);display:flex;align-items:center;justify-content:center;font-size:1.4rem;font-weight:800;color:white;flex-shrink:0;border:2px solid rgba(255,255,255,0.2);}
  .bg-card-name{font-size:1rem;font-weight:700;color:white;}
  .bg-card-sub{font-size:0.78rem;color:rgba(255,255,255,0.6);margin-top:3px;}
  .bg-avail{margin-left:auto;font-size:0.7rem;font-weight:700;padding:4px 10px;border-radius:20px;flex-shrink:0;text-transform:uppercase;letter-spacing:0.04em;}
  ...
  .bg-langs{font-size:0.78rem;color:#6b7280;margin-bottom:8px;}
  ...
  .bg-rate-val{font-size:1rem;font-weight:700;color:#0a2818;}
  .bg-view-btn{background:#16a34a;color:white;border:none;border-radius:10px;padding:9px 16px;font-size:0.8rem;font-weight:600;cursor:pointer;font-family:'Roboto',sans-serif;text-decoration:none;transition:all 0.2s;display:inline-block;}
  .bg-view-btn:hover{background:#15803d;transform:translateY(-1px);}
  .bg-empty{text-align:center;padding:64px 24px;}
  .bg-empty h3{font-size:1.3rem;color:#0a2818;margin-bottom:8px;}
  ...
  .bg-cta{background:linear-gradient(135deg,#0a2818 0%,#1a4a2a 100%);text-align:center;padding:80px 24px;margin-top:40px;}
  .bg-cta h2{font-size:2rem;font-weight:700;color:white;margin-bottom:12px;}
  ...
  .bg-cta-a{background:#16a34a;color:white;padding:12px 28px;border-radius:10px;font-weight:700;font-size:0.875rem;text-decoration:none;transition:all 0.2s;}
  .bg-cta-a:hover{background:#15803d;transform:translateY(-2px);}
  .bg-cta-b{background:rgba(255,255,255,0.1);color:white;padding:12px 28px;border-radius:10px;font-weight:600;font-size:0.875rem;text-decoration:none;border:1px solid rgba(255,255,255,0.25);backdrop-filter:blur(8px);transition:all 0.2s;}
...
const BrowseGuides = () => {
  ...
  // Re-apply filters whenever raw list or filters change
  ...
  const fetchGuides = async () => {
    try {
      ...
      console.log('[BrowseGuides] received', raw.length, 'guides');
      setAllGuides(raw);
    } catch (err) {
  ...
  // All fields are now flat on the guide object (normalized by the controller)
  const applyFilters = (raw) => {
    const filtered = raw.filter(g => {
      ...
      const matchesRating =
        !filters.minRating || (g.rating || 0) >= Number(filters.minRating);

      const matchesPrice =
        !filters.maxPrice || !g.hourlyRate || g.hourlyRate <= Number(filters.maxPrice);
        ...
        <section className="bg-hero">
          ...
          <div className="bg-stats">
            <div className="bg-stat">
              <span className="bg-stat-num">{loading ? '…' : allGuides.length}</span>
              <span className="bg-stat-label">Certified Guides</span>
            </div>
        ...
        <div className="bg-body">
          ...
          <aside className="bg-sidebar">
            ...
            <div className="bg-filter-section">
              ...
              <input type="text" name="name" className="bg-filter-input"
            ...
            <div className="bg-filter-section">
              ...
              <input type="number" name="maxPrice" className="bg-filter-input"
          ...
          <div className="bg-results">
            <div className="bg-results-head">
              <span className="bg-results-count">
                {guides.length} guide{guides.length !== 1 ? 's' : ''} found
              </span>
            </div>

            {error && (
              <div style={{ background: '#fef2f2', color: '#dc2626', padding: '16px', borderRadius: '10px', marginBottom: '16px', fontSize: '14px' }}>
                {error} <button onClick={fetchGuides} style={{ marginLeft: 8, color: '#16a34a', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Retry</button>
              </div>
            )}
            ...
            {loading ? (
              ...
              <div className="bg-empty">
                ...
                <p>
                  {allGuides.length === 0
                    ...
                    : <>Try adjusting your filters or <button onClick={resetFilters} style={{ color: '#16a34a', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>clear all filters</button></>
                  }
                </p>
              </div>
            ) : (
              <div className="bg-grid">
                {guides.map(guide => {
                  // All fields are flat — no more guideProfile nesting
                  ...
                  return (
                    <div key={guide._id} className="bg-card">
                      <div className="bg-card-head">
                        {guide.profileImage
                          ? <img src={guide.profileImage} alt={name} style={{ width:56,height:56,borderRadius:'50%',objectFit:'cover',border:'2px solid rgba(255,255,255,0.2)',flexShrink:0 }} />
                          : <div className="bg-avatar">{initial}</div>
                        }
                        <div style={{ minWidth: 0 }}>
                          <div className="bg-card-name">{name}</div>
                          {guide.yearsExperience > 0 && (
                            <div className="bg-card-sub">🗓️ {guide.yearsExperience} yrs experience</div>
                          )}
                          {guide.licenseNumber && (
                            <div className="bg-card-sub">🪪 Licensed</div>
                          )}
                        </div>
                        <span className={`bg-avail ${guide.availability ? 'available' : 'unavailable'}`}>
                      ...
                      <div className="bg-card-body">
                        {guide.rating > 0 && (
                          <div className="bg-rating">
                            <span className="bg-stars">{'★'.repeat(Math.floor(guide.rating))}</span>
                            <span className="bg-rating-val">({guide.rating.toFixed(1)})</span>
                          </div>
                        )}

                        {guide.bio && <p className="bg-bio">{guide.bio}</p>}

                        {guide.specializations?.length > 0 && (
                          <div className="bg-tags">
                            {guide.specializations.slice(0, 3).map((s, i) => (
                              <span key={i} className="bg-tag">{s}</span>
                            ))}
                            {guide.specializations.length > 3 && (
                              <span className="bg-tag">+{guide.specializations.length - 3}</span>
                            )}
                          </div>
                        )}

                        {guide.languages?.length > 0 && (
                          <p className="bg-langs">
                          ...
                          <div className="bg-rates">
                            {guide.hourlyRate > 0 && (
                              <div className="bg-rate">
                                ...
                                <span className="bg-rate-val">${guide.hourlyRate}</span>
                              </div>
                            )}
                            {guide.dailyRate > 0 && (
                              <div className="bg-rate">
                                <span className="bg-rate-label">Daily</span>
                                <span className="bg-rate-val">${guide.dailyRate}</span>
                              </div>
                            )}
                          </div>
                          <Link to={`/guides/${guide._id}`} className="bg-view-btn">
                            View Profile
                          </Link>
                        </div>
                      </div>
