...
const STYLES = `
  ...
  .ga-root { font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .ga-note-input { width:100%; padding:10px 14px; border:1.5px solid #d1fae5; border-radius:10px; font-size:13px; font-family:inherit; color:#0f172a; outline:none; resize:vertical; transition:border 0.15s; }
...
export default function GuideApplications() {
  ...
  const fetchApps = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${API}/guide-applications?status=${filter}`, { headers: { Authorization: `Bearer ${token()}` } });
  ...
  const handleApprove = async (id) => {
    setProcessing(true);
    try {
      await axios.put(`${API}/guide-applications/${id}/approve`, { note: actionNote }, { headers: { Authorization: `Bearer ${token()}` } });
      notify('✓ Guide approved successfully.');
      setSelected(null);
      fetchApps();
    } catch (err) { notify(err.response?.data?.message || 'Failed to approve'); }
    finally { setProcessing(false); setActionNote(''); }
  };

  const handleReject = async (id) => {
    if (!actionNote.trim()) return notify('⚠️ Please provide rejection reason');
    setProcessing(true);
    try {
      await axios.put(`${API}/guide-applications/${id}/reject`, { reason: actionNote }, { headers: { Authorization: `Bearer ${token()}` } });
      notify('Application rejected.');
      setSelected(null);
      fetchApps();
    } catch (err) { notify(err.response?.data?.message || 'Failed to reject'); }
    finally { setProcessing(false); setActionNote(''); }
  };
      ...
      <div className="ga-root">
        ...
        <div className="ga-filters">
          {['pending', 'approved', 'rejected'].map(f => (
            <button key={f} className={`ga-filter-btn${filter === f ? ' active' : ''}`} onClick={() => setFilter(f)}>
              {f === 'pending' ? '⏳' : f === 'approved' ? '✅' : '❌'} {f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>

        {loading ? (
          <div className="ga-loading"><div className="ga-spinner" /><p>Loading applications…</p></div>
        ) : applications.length === 0 ? (
          <div className="ga-empty">
            <div style={{ fontSize: '2.5rem', marginBottom: 12 }}>🧭</div>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#0a2818', marginBottom: 6 }}>No {filter} applications</div>
              ...
              return (
                <div key={app._id} className="ga-card">
                  <div className="ga-card-top">
                    <div>
                      <div className="ga-name">{app.user?.firstName} {app.user?.lastName}</div>
                      <div className="ga-email">{app.user?.email}</div>
                      {app.user?.phone && <div className="ga-email">📱 {app.user.phone}</div>}
                    </div>
                    <span className={`ga-badge ${sc.cls}`}>{sc.label}</span>
                  </div>
                  <div className="ga-info-row"><strong>Experience:</strong> {app.yearsExperience} years</div>
                  <div className="ga-info-row"><strong>Languages:</strong> {app.languages?.join(', ') || 'N/A'}</div>
                  {app.specializations?.length > 0 && (
                    <div className="ga-tags">
                      {app.specializations.map(s => <span key={s} className="ga-tag">{s}</span>)}
                    </div>
                  )}
                  {app.licenseNumber && <div className="ga-info-row"><strong>License:</strong> {app.licenseNumber}</div>}
                  ...
                  <div className="ga-btns">
                    <button className="ga-btn ga-btn-view" onClick={() => { setSelected(app); setActionNote(''); }}>View Details</button>
                    {app.applicationStatus === 'pending' && (
                      <>
                        <button className="ga-btn ga-btn-approve" onClick={() => handleApprove(app._id)} disabled={processing}>✓ Approve</button>
                        <button className="ga-btn ga-btn-reject" onClick={() => { setSelected(app); setActionNote(''); }}>✕</button>
                      </>
                    )}
                  </div>
                </div>
        ...
        {selected && (
          <div className="ga-overlay" onClick={e => e.target === e.currentTarget && setSelected(null)}>
            <div className="ga-modal">
              <div className="ga-modal-head">
                <div className="ga-modal-title">
                  {selected.user?.firstName} {selected.user?.lastName}
                  <span className={`ga-badge ${STATUS[selected.applicationStatus]?.cls || 'pending'}`} style={{ marginLeft: 10, fontSize: 11 }}>
              ...
              <div className="ga-modal-body">
                <div className="ga-modal-section">
                  ...
                  <div className="ga-modal-value">📧 {selected.user?.email}</div>
                  {selected.user?.phone && <div className="ga-modal-value">📱 {selected.user.phone}</div>}
                </div>
                <div className="ga-modal-section">
                  <span className="ga-modal-label">Experience & Qualifications</span>
                  <div className="ga-modal-value">🗓️ {selected.yearsExperience} years of experience</div>
                  {selected.licenseNumber && <div className="ga-modal-value">📋 License: {selected.licenseNumber}</div>}
                </div>
                <div className="ga-modal-section">
                  <span className="ga-modal-label">Specializations</span>
                  <div className="ga-tags">{selected.specializations?.map(s => <span key={s} className="ga-tag">{s}</span>)}</div>
                </div>
                <div className="ga-modal-section">
                  <span className="ga-modal-label">Languages</span>
                  <div className="ga-tags">{selected.languages?.map(l => <span key={l} className="ga-tag">{l}</span>)}</div>
                </div>
                {selected.bio && (
                  <div className="ga-modal-section">
                    <span className="ga-modal-label">Bio</span>
                    <div className="ga-modal-value" style={{ lineHeight: 1.6 }}>{selected.bio}</div>
                  </div>
                )}
                {selected.applicationStatus === 'pending' && (
                  <div className="ga-modal-section">
                    <span className="ga-modal-label">Note / Rejection Reason</span>
                    <textarea className="ga-note-input" rows={3}
              ...
              {selected.applicationStatus === 'pending' && (
                <div className="ga-modal-btns">
                  <button className="ga-modal-approve" onClick={() => handleApprove(selected._id)} disabled={processing}>
                    {processing ? 'Processing…' : '✓ Approve Guide'}
                  </button>
                  <button className="ga-modal-reject" onClick={() => handleReject(selected._id)} disabled={processing}>
