...
const EMPTY = { title:'', category:'Trekking', difficulty:'Moderate', duration:'', groupSize:{ min:1, max:15 }, price:{ amount:'', currency:'NPR', perPerson:true }, description:'', highlights:[''], itinerary:[{ ...EMPTY_DAY }], includes:[], excludes:[''], startLocation:'', endLocation:'', bestSeason:[], images:[''], isActive:true, isFeatured:false };
...
const STYLES = `
  ...
  .mp-search { border:none; outline:none; font-size:13px; font-family:'Plus Jakarta Sans',sans-serif; color:#0f172a; background:transparent; flex:1; }
  .mp-search::placeholder { color:#9ca3af; }
  .mp-add-btn { display:flex; align-items:center; gap:6px; padding:10px 18px; background:#16a34a; color:#fff; border:none; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; white-space:nowrap; transition:background 0.15s; }
  ...
  .mp-fsel select { border:none; background:none; outline:none; font-size:12px; font-weight:500; color:#374151; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; padding:0; }
  ...
  .mp-btn-edit { display:flex; align-items:center; gap:5px; padding:6px 12px; background:#f0fdf4; color:#15803d; border:1px solid #d1fae5; border-radius:7px; font-size:12px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all 0.15s; }
  .mp-btn-edit:hover { background:#dcfce7; }
  .mp-btn-del  { display:flex; align-items:center; gap:5px; padding:6px 12px; background:#fef2f2; color:#dc2626; border:1px solid #fecaca; border-radius:7px; font-size:12px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all 0.15s; }
  .mp-btn-del:hover { background:#fee2e2; }
  .mp-back-btn { display:flex; align-items:center; gap:6px; padding:9px 16px; background:#f8faf8; color:#374151; border:1.5px solid #e5f0e8; border-radius:9px; font-size:13px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all 0.15s; }
  .mp-back-btn:hover { border-color:#16a34a; color:#15803d; }
  .mp-save-btn { display:flex; align-items:center; gap:6px; padding:10px 20px; background:#16a34a; color:#fff; border:none; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:background 0.15s; }
  ...
  .mp-ftab  { padding:7px 14px; border-radius:7px; border:none; cursor:pointer; font-size:12px; font-weight:600; font-family:'Plus Jakarta Sans',sans-serif; background:transparent; color:#6b7280; transition:all .13s; }
  ...
  .mp-inp   { padding:10px 13px; border:1.5px solid #d1fae5; border-radius:9px; font-size:13px; color:#0f172a; outline:none; font-family:'Plus Jakarta Sans',sans-serif; width:100%; transition:border 0.15s; }
  .mp-inp:focus { border-color:#16a34a; box-shadow:0 0 0 3px rgba(22,163,74,0.08); }
  .mp-textarea { padding:10px 13px; border:1.5px solid #d1fae5; border-radius:9px; font-size:13px; color:#0f172a; outline:none; font-family:'Plus Jakarta Sans',sans-serif; width:100%; resize:vertical; min-height:90px; transition:border 0.15s; }
  .mp-textarea:focus { border-color:#16a34a; }
  .mp-select   { padding:10px 13px; border:1.5px solid #d1fae5; border-radius:9px; font-size:13px; color:#0f172a; outline:none; font-family:'Plus Jakarta Sans',sans-serif; width:100%; background:#fff; transition:border 0.15s; }
  .mp-select:focus { border-color:#16a34a; }
  .mp-tag { padding:6px 13px; border-radius:20px; border:1.5px solid #d1fae5; font-size:12px; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; background:#f8faf8; color:#374151; transition:all .13s; }
...
export default function ManagePackages() {
  ...
  const startEdit = (p) => {
    ...
    setForm({ ...EMPTY, ...p, title:p.name||p.title||'', price:{ amount:String(displayPrice(p)), currency:'NPR', perPerson:true }, groupSize:{ min:1, max:p.maxGroupSize||15 }, highlights:p.highlights?.length?p.highlights:[''], excludes:p.excludes?.length?p.excludes:[''], itinerary:p.itinerary?.length?p.itinerary:[{ ...EMPTY_DAY }], includes:incl, bestSeason:p.bestSeason||[], images:Array.isArray(p.images)&&p.images.length?p.images:[''] });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { name:form.title.trim(), description:form.description.trim(), duration:Number(form.duration), price:Number(form.price.amount), maxGroupSize:Number(form.groupSize?.max)||15, difficulty:form.difficulty==='Strenuous'?'Challenging':form.difficulty, category:form.category, images:form.images.filter(i=>i.trim()), isActive:form.isActive, isFeatured:form.isFeatured, startLocation:form.startLocation, endLocation:form.endLocation, bestSeason:form.bestSeason, highlights:form.highlights.filter(h=>h.trim()), excludes:form.excludes.filter(e=>e.trim()), itinerary:form.itinerary.map(d=>({ day:d.day, title:d.title, description:d.description, elevation:d.elevation, distance:d.distance })), includes:{ accommodation:form.includes.includes('Accommodation'), transport:form.includes.includes('Transport'), guide:form.includes.includes('Guide'), meals:form.includes.includes('Meals (B/L/D)')?'All meals':'', activities:form.includes.filter(i=>!['Accommodation','Transport','Guide','Meals (B/L/D)'].includes(i)) } };
      if (editId) { await axios.put(`${API}/packages/${editId}`, payload, { headers:{ Authorization:`Bearer ${token()}` } }); notify('✓ Package updated successfully'); }
      else        { await axios.post(`${API}/packages`, payload, { headers:{ Authorization:`Bearer ${token()}` } }); notify('✓ Package created successfully'); }
      fetchPackages(); setTab('list');
    } catch (err) { notify(`⚠️ ${err.response?.data?.message || err.response?.data?.error || 'Failed to save'}`); }
  ...
  const filtered = packages.filter(p => {
    ...
    return (!search||name.includes(search.toLowerCase())||p.category?.toLowerCase().includes(search.toLowerCase())) && (catFilter==='All Categories'||p.category===catFilter) && (diffFilter==='All Difficulties'||p.difficulty===diffFilter) && (statusFilter==='All Status'||(statusFilter==='Active'?p.isActive!==false:p.isActive===false));
  });

  const FORM_TABS = ['basic','itinerary','includes','images'];
  const FTAB_LABELS = { basic:'Basic Info', itinerary:'Itinerary', includes:'What\'s Included', images:'Images' };
      ...
      <div className="mp-root">
        ...
        {tab === 'list' ? (
          <>
            ...
            <div className="mp-filterbar">
              <div className="mp-filterbar-r1">
                <div className="mp-search-wrap"><span style={{ color:'#9ca3af' }}>🔍</span><input className="mp-search" placeholder="Search by name, category…" value={search} onChange={e=>setSearch(e.target.value)} /></div>
              ...
              {loading ? (
                ...
                <div style={{ overflowX:'auto' }}>
                  <table className="mp-table">
                    <thead><tr><th>Package</th><th>Category</th><th>Difficulty</th><th>Duration</th><th>Price</th><th>Status</th><th></th></tr></thead>
                    <tbody>
                      {filtered.map(p => (
                        <tr key={p._id}>
                          <td>
          ...
          <>
            ...
            <div className="mp-ftabs">{FORM_TABS.map(t=><button key={t} className={`mp-ftab${formTab===t?' on':''}`} onClick={()=>setFormTab(t)}>{FTAB_LABELS[t]}</button>)}</div>
