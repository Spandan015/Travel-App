...
const getItemPrice = (type, item) => {
  ...
  if (type === 'package') return item?.price          || 0;
...
const getItemLocation = (type, item) => {
  ...
  if (type === 'package') return item?.destinations?.[0]?.name || '';
...
export default function BookingModal({ type = 'hotel', item, onClose }) {
  ...
  const baseAmount   = type === 'hotel'
    ...
    : baseRate; // packages/treks: flat price
  ...
  // ── Step 2: create booking, then redirect to eSewa ─────────────────────────
  const handlePayWithEsewa = async () => {
    ...
    try {
      ...
      const bookingPayload = type === 'hotel'
        ? {
            hotelId:        item._id,
      ...
      // 3️⃣ Redirect to eSewa sandbox
      setStep('paying');
      setTimeout(() => redirectToEsewa(esewaRes), 500); // short delay so UI shows "Redirecting…"
      ...
      <div style={S.modal}>
          ...
          <div style={S.body}>
                ...
                <div style={S.row2}>
                  <div style={S.field}>
                    ...
                    <input
                      ...
                      onChange={e => {
                        set('checkInDate', e.target.value);
                        if (form.checkOutDate && form.checkOutDate <= e.target.value) set('checkOutDate', '');
                      }}
                    />
                  </div>
                  <div style={S.field}>
            ...
            <section style={S.section}>
              ...
              <div style={S.counterItem}>
                <div>
                  <div style={S.counterLabel}>Adults</div>
                  <div style={S.counterSub}>Age 13+</div>
                </div>
                {counter('adults', 1, 10)}
              </div>
              <div style={S.counterItem}>
                <div>
                  <div style={S.counterLabel}>Children</div>
                  <div style={S.counterSub}>Age 0–12</div>
                </div>
        ...
        {step === 'summary' && (
          <div style={S.body}>
            ...
            <button style={S.backBtn} onClick={() => setStep('details')}>
              ← Edit Details
            </button>
          </div>
        )}
...
const S = {
  ...
  nightBadge: {
    ...
    padding: '5px 14px', borderRadius: 20, marginTop: 10,
  },
  counterItem: {
  ...
  priceRow: {
    ...
    fontSize: 13, color: '#64748b', marginBottom: 7,
  },
  totalRow: {
    display: 'flex', justifyContent: 'space-between',
  ...
  esewaBtn: {
    ...
    boxShadow: '0 4px 16px rgba(96,187,70,0.35)',
    transition: 'all 0.2s',
  },
  backBtn: {
  ...
  spinner: {
    ...
    borderRadius: '50%', animation: 'bm-spin 0.9s linear infinite',
    margin: '0 auto 20px',
  },
};
